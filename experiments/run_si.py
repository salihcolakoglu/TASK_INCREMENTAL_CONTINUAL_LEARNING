"""
Run SI (Synaptic Intelligence) baseline on task-incremental continual learning benchmarks.

Usage:
    python experiments/run_si.py --dataset split_mnist --n_tasks 5 --si_lambda 1.0
    python experiments/run_si.py --dataset split_cifar10 --n_tasks 5 --si_lambda 1.0
    python experiments/run_si.py --dataset split_cifar100 --n_tasks 10 --si_lambda 1.0
"""
...
from src.baselines import SynapticIntelligenceTrainer
from src.utils import get_dataset


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='SI baseline for continual learning')
    ...
    # SI-specific arguments
    parser.add_argument('--si_lambda', type=float, default=1.0,
                       help='SI regularization strength (λ)')
    parser.add_argument('--si_epsilon', type=float, default=0.001,
                       help='SI numerical stability constant (ε)')
    parser.add_argument('--damping', type=float, default=0.1,
                       help='SI damping parameter (c) for normalizing importance')
...
def main():
    ...
    print("="*70)
    print("SI BASELINE - TASK-INCREMENTAL CONTINUAL LEARNING")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    print(f"\n  SI lambda (λ): {args.si_lambda}")
    print(f"  SI epsilon (ε): {args.si_epsilon}")
    print(f"  SI damping (c): {args.damping}")
    print()
    ...
    # Save final results
    if args.save_checkpoints:
        results_file = os.path.join(args.checkpoint_dir, f'si_lambda{args.si_lambda}_results.txt')
        with open(results_file, 'w') as f:
            f.write("SI Baseline Results\n")
            f.write("="*70 + "\n\n")
