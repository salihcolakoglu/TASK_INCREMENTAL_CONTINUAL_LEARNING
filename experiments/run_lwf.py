"""
Run LwF (Learning without Forgetting) baseline on task-incremental continual learning benchmarks.

Usage:
    python experiments/run_lwf.py --dataset split_mnist --n_tasks 5 --lwf_lambda 1.0 --temperature 2.0
    python experiments/run_lwf.py --dataset split_cifar10 --n_tasks 5 --lwf_lambda 1.0
    python experiments/run_lwf.py --dataset split_cifar100 --n_tasks 10 --lwf_lambda 1.0
"""
...
from src.baselines import LwFTrainer
from src.utils import get_dataset


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='LwF baseline for continual learning')
    ...
    parser.add_argument('--lwf_lambda', type=float, default=1.0,
                       help='LwF distillation loss weight (λ)')
    parser.add_argument('--temperature', type=float, default=2.0,
                       help='Temperature for knowledge distillation (T)')
...
def main():
    ...
    print("="*70)
    print("LwF BASELINE - TASK-INCREMENTAL CONTINUAL LEARNING")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    if args.save_checkpoints:
        results_file = os.path.join(
            args.checkpoint_dir,
            f'lwf_lambda{args.lwf_lambda}_temp{args.temperature}_results.txt'
        )
        with open(results_file, 'w') as f:
            f.write("LwF Baseline Results\n")
            f.write("="*70 + "\n\n")
