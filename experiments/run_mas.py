"""
Run MAS (Memory Aware Synapses) baseline on task-incremental continual learning benchmarks.

Usage:
    python experiments/run_mas.py --dataset split_mnist --n_tasks 5 --mas_lambda 1.0
    python experiments/run_mas.py --dataset split_cifar10 --n_tasks 5 --mas_lambda 1.0
    python experiments/run_mas.py --dataset split_cifar100 --n_tasks 10 --mas_lambda 1.0
"""
...
from src.baselines import MASTrainer
from src.utils import get_dataset


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='MAS baseline for continual learning')
    ...
    # MAS-specific arguments
    parser.add_argument('--mas_lambda', type=float, default=1.0,
                       help='MAS regularization strength (λ)')
    parser.add_argument('--num_samples', type=int, default=200,
                       help='Number of samples for importance estimation')
...
def main():
    ...
    print("="*70)
    print("MAS BASELINE - TASK-INCREMENTAL CONTINUAL LEARNING")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    # Save final results
    if args.save_checkpoints:
        results_file = os.path.join(args.checkpoint_dir, f'mas_lambda{args.mas_lambda}_results.txt')
        with open(results_file, 'w') as f:
            f.write("MAS Baseline Results\n")
            f.write("="*70 + "\n\n")
