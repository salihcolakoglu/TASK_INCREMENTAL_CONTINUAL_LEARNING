"""
Run Fine-tuning baseline on task-incremental continual learning benchmarks.

This script demonstrates the catastrophic forgetting problem by training
a model sequentially on tasks without any forgetting mitigation.

Usage:
    python experiments/run_finetune.py --dataset split_mnist --n_tasks 5
    python experiments/run_finetune.py --dataset split_cifar10 --n_tasks 5
    python experiments/run_finetune.py --dataset split_cifar100 --n_tasks 10
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Fine-tuning baseline for continual learning')
...
def main():
    ...
    print("="*70)
    print("FINE-TUNING BASELINE - TASK-INCREMENTAL CONTINUAL LEARNING")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    # Save final results
    if args.save_checkpoints:
        results_file = os.path.join(args.checkpoint_dir, 'final_results.txt')
        with open(results_file, 'w') as f:
            f.write("Fine-tuning Baseline Results\n")
            f.write("="*70 + "\n\n")
